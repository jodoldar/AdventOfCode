 ...
 * @brief Day 4 - Part 2 of the Advent of Code 2021
 * @version 0.1
 * @date 2021-12-05
 * 
    ...
    if (!siReadAndProcessInput("input.txt", vAppearedNumbers, vCardboards))
    {
        ...
        for (auto &&iNumber : vAppearedNumbers)
        {
            ...
            for (size_t iPos = 0; iPos < vCardboards.size(); iPos++)
            {
                ...
                if (!vCardboards[iPos].bHasWon && vCardboards[iPos].isWinner())
                {
                    cout << "Found a new winning card. Score -> " << vCardboards[iPos].getScore(iNumber) << endl;
                }
